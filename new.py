...
def generate_explanation(row):
    ...
    if row["ml_confidence"] > 0.7:
        decision_source = "ML-driven"
    elif row["ml_confidence"] >= 0.5:
        decision_source = "Hybrid"
    else:
        decision_source = "Rule-based"

    # Amount anomaly
    if row["amount_vs_diag_ratio"] > 2:
        reasons.append("Amount significantly higher than diagnosis average")

    # Night
    if row["is_night"] == 1:
        reasons.append("Claim at unusual night hours")

    # Repeat
    if row["patient_claims_today"] > 1:
        reasons.append("Repeated claims for same patient")

    # Provider anomaly
    if row["provider_claim_count"] < 5:
        reasons.append("Low-frequency provider (less historical data)")

    # ML reasoning
    if row["ml_prediction"] == "fraud" and row["ml_confidence"] > 0.6:
        reasons.append("ML model indicates fraud pattern")

    # Fallback
    if len(reasons) == 0:
        if row["ml_prediction"] == "fraud":
            reasons.append("Detected via learned fraud patterns")
        else:
            reasons.append("No anomaly detected")

    return decision_source, reasons[:3]
...
If ml_conf between 0.6 and 0.7:
...
Step 4— Output
Output **exactly one word** on a new line:
fraud  
or  
non-fraud
...
def react_hybrid_classify_fast(row):
    ...
    try:
        ...
        label = clean_label(response.text)
        return label
    except Exception:
...
for batch_idx in tqdm(range(0, len(eval_df), BATCH_SIZE), desc="ReAct Batches"):
    ...
    eval_df["final_prediction"] = react_hybrid_preds
    time.sleep(SLEEP_TIME)

# -------------------------------------------------------
# GENERATE EXPLANATIONS
# -------------------------------------------------------
explanations = [generate_explanation(row) for _, row in eval_df.iterrows()]

eval_df["decision_source"] = [e[0] for e in explanations]
eval_df["reason_1"] = [e[1][0] for e in explanations]
eval_df["reason_2"] = [e[1][1] if len(e[1]) > 1 else "" for e in explanations]
eval_df["reason_3"] = [e[1][2] if len(e[1]) > 2 else "" for e in explanations]
...
# -------------------------------------------------------
print("\n\n===== EXPLAINABILITY OUTPUT SAMPLES =====")

fraud_cases = eval_df[eval_df["final_prediction"] == "fraud"].head(2)
nonfraud_cases = eval_df[eval_df["final_prediction"] == "non-fraud"].head(2)

def print_case(row, idx):
    print(f"\n--- Case {idx} ---")
    print(f"Patient ID: {row['patient_id']}")
    print(f"Prediction: {row['final_prediction']}")
    print(f"Confidence: {row['ml_confidence']:.2f}")
...
for i, (_, row) in enumerate(fraud_cases.iterrows(), 1):
    print_case(row, i)

for i, (_, row) in enumerate(nonfraud_cases.iterrows(), 3):
    print_case(row, i)